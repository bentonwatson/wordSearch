 ...
 * 
 *         Pseudo Code: 
 *         1. Set the puzzle ID (if exists) through PuzzlePicker 
 *         2. Read the puzzle.txt and populate the
 ...
 * 
 */
public class WordSearchPuzzlePlayer extends JPanel implements MouseListener,
		ComponentListener
{
	...
	private static ProgressTracker progress;
	...
	public WordSearchPuzzlePlayer(PuzzleCollection inPuzzleCollection)
	{
	...
	private void buildGui(Puzzle a_puzzle)
	{
		...
		try
		{
			background = ImageIO.read(WordSearchPuzzlePlayer.class
	...
	public static void main(String[] args)
	{
		...
		// ------------------
		/*
		 REQUIREMENTS for SE 5 (Group Assignment):
		 1. Instead of starting from 001 all the times,
		 the game should start from the last incomplete game.
		
		 2. The game status / progress is tracked in the following file
		 (This file name is specified in the Config.java).
		 $HOME/ICS499/<GameName>_Progress.txt
		 
		 3. Optionally, this file can also be a BINARY file so that
		 users can not manually change the file contents. You can specify
		 the file format through another CONFIG variable.
		 
		 4. The progress should monitor the following attributes
		 ID of the puzzle / game completed by the user.
		 The Date on which the user completed the game
		 And Time it has taken to complete the game
		 
		 5. When the game/puzzle is started again, the system should rely on
		 $HOME/ICS499/<GameName>_Progress.txt to determine which game to present to the user
		 
		 6. Design Constraint: Ensure that the interface and implementation you come up with
		 can be used in your final project as well.
		 */
		 
		//TODO this is where we enter the id of the game we need
		progress = new ProgressTracker();
		String gameId = progress.getCurrentGameId();
		PuzzlePicker pp1 = new PuzzlePicker(gameId);//this class not really necessary but left because it was pre-existing
		
		// populates the puzzle collection
		PuzzleCollection collection = new PuzzleCollection();
		collection.setCurrentId(pp1.getId());
		
		JFrame frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		WordSearchPuzzlePlayer ppframe = new WordSearchPuzzlePlayer(collection);
